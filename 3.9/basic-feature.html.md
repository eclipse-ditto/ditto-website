...
> **TL;DR**: A Feature has an ID, properties (current state), optional desired
properties (target state), and an optional definition that links it to a type model.
...
> **Note:** Ditto does not contain a built-in type system and does not validate that properties
