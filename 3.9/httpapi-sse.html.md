...
> **TL;DR**: Open an SSE connection to `/api/2/things` with `Accept: text/event-stream` to stream change notifications, or to `/api/2/search/things` to stream search results. Filter with `namespaces`, `filter`, `fields`, and `extraFields` parameters.
