...
> **TL;DR**: The HTTP API auto-generates endpoints from your Thing's JSON structure, supports partial reads/writes, merge updates via `PATCH`, and conditional requests with ETags for optimistic locking.
...
Ditto extends RFC 7396 with regex-based field removal. Use the syntax `{{ ~<regex>~ }}` with a `null` value to delete matching keys before applying the rest of the patch:
