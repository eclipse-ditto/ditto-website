...
> **TL;DR**: Use pre-authentication when a reverse proxy handles user verification and passes the identity via HTTP header. Use OpenID Connect when you need JWT-based authentication from an OIDC provider like Keycloak.
...
* `auth-subjects`: A list of placeholder templates evaluated against incoming JWTs. Each entry generates an authorization subject. Entries with unresolvable placeholders are ignored. Defaults to `{{ jwt:sub }}` when not provided.
