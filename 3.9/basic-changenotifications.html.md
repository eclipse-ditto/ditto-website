...
> **TL;DR**: Subscribe to change notifications via WebSocket, Server Sent Events (SSE),
or connections. Filter by namespace or RQL expression to receive only the events you care about.
...
> **Note:** All filters are specified in URL query format, so their values should be URL-encoded.
...
> **Note:** The RQL filter applies to the *modified* data by default. Unchanged data is only
