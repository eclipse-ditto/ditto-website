...
> **TL;DR**: Create a Thing with a `POST` request, read it with `GET`, and update individual
properties with `PUT` -- all using standard HTTP.
...
> **Note:** You can also use `POST` to `/api/2/things` without specifying an ID -- Ditto will auto-generate one.
