...
> **TL;DR**: Set `validateCertificates: true` and provide a `ca` certificate to verify server identity. Add `credentials` with type `client-cert` to authenticate Ditto with a client certificate.
