...
> **TL;DR**: Override any Ditto config value by setting an environment variable (when the config uses `${?ENV_NAME}` syntax) or by passing a Java system property (`-Dkey=value`) to the service process.
...
> **Note:** If you disable encryption later, keep the symmetric key in the configuration. Without it, previously encrypted values cannot be decrypted, and you would need to manually re-enter the encrypted connection fields.
...
- **Resume aborted migration:**
  ...
  > **Note:** If the previous migration already completed, was never started, or only ran as a dry run (which does not persist progress), the resume command returns `200 OK` with `phase: \"already_completed\"` instead of starting a new migration. This makes resume safe to call idempotently.
