...
from that array, each `{{ jwt:<path> }}` placeholder resolves independently across all array elements
...
* `{{ jwt:authz/reseller }}#{{ jwt:authz/service_provider }}#{{ jwt:authz/roles }}` - resolves to **4** entries including incorrect mixes like `"io.test#acme#resellerAdmin"` and `"io.test##op"`

Using `fn:format()` (keeps fields correlated per object):
* `{{ jwt:authz | fn:format('{reseller}#{service_provider}#{roles}') }}` - resolves to only the **2 correct** entries: `"io.test#acme#op"` and `"io.test##resellerAdmin"`
...
* `{{ jwt:authz | fn:format('{reseller}:{service_provider}:{#permissions}{resource}:{#actions}{.}{/actions}{/permissions}') }}` - resolves to **3** entries: `"io.test:acme:things:read"`, `"io.test:acme:things:write"` and `"io.test:acme:policies:read"`
