...
[JSON Schema: jsonschema/connection.json](../jsonschema/connection.json)
...
`{{connection:id}}` placeholder followed by a colon. For example:
```json
{
  "addresses": [
    "<source>"
...
`{{connection:id}}` placeholder followed by a colon.<br/>
...
`{{ placeholder }}`. Have a look at the [placeholders concept](basic-placeholders.html) for 
...
message to provide necessary information about the original issuer of the message. 
  ...
  > **Important:** Only use this kind of placeholder if you trust the source of the message. The value from the header is used as the **authorized subject**." additionalStyle="
...
You can use the placeholders `{{ thing:id }}`, `{{ thing:namespace }}` 
and `{{ thing:name }}` in the target address for this purpose.
