...
> **Note:** Find the HTTP API reference at [Policies resources](http-api-doc.html?urls.primaryName=api2#/Policies).
...
> **Tip:** Write permission at the policy root resource (i.e. `policy:/`) allows to manage the
  policy itself.
Find an [example](#example) at the end of the page.
...
> **Tip:** Make use of this action in order to copy your existing permissions for a pre-configured connection 
...
The Policy resource (addressable as `policy:/`) defines the access control for the Policy itself.
  ...
  > **Tip:** Please make sure to define at least one user (for which you have the credentials) with
...
Policy.
  ...
  > **Note:** In case you want to re-use a policy for various things, please make sure to name the
...
> **Tip:** The resources `message:/inbox` and `message:/outbox` do not address feature-related messages. 
...
> **Note:** The sanity check, ensuring that at least one subject has WRITE permission on the policy's root resource, is *not* applied for policies defining policy imports. So pay attention that you don't lock yourself out when creating/modifying such policies.
...
the root resources.
  ...
  > **Tip:** The easiest way to create a Policy is to copy the model schema provided at the
...
![Policy Example](images/pages/basic/policy-example.png)
*Example Thing with link to a Policy ID*

Your Policy then might look like the following:

![Policy Example 2](images/pages/basic/policy-example-2.png)
*Example Policy*
...
> **Tip:** As soon as a sophisticated policy is described, you will only need to add a further **subject** entry to have for example a new group of users equally empowered as the initial one.
