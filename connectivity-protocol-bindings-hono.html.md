...
> **Note:** A Hono connection is associated with _one_ Hono tenant. That means for each Hono tenant a separate Hono connection
...
`<honoTenantId>` will be replaced by the value of `specificConfig.honoTenantId` or, if not set,
...
* `command` -> `hono.command.<honoTenantId>/<thingId>`

`<honoTenantId>` will be replaced by the value of `specificConfig.honoTenantId` or, if not set,
by the connection id. `<thingId>` is substituted by the thing ID value.
